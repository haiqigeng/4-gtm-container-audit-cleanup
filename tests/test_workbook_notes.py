...
def fixture():
    ...
    locked = {"containerVersion": {"tag": [
        {"tagId": "1", "templateData": "a" * 82000},
        {"tagId": "3", "notes": "obsolete"},
    ]}}
    return {
        ...
        "operations": [{"operation_id": "OP-ONE", "source_reconciled_decision_ids": ["CD-ONE"],
                        ...
                        "changes": [{"object_key": "tag:1", "json_path": "$.templateData",
                                     "before_source_sha256": stable_hash(locked, 64), "after": "b" * 81000}]}],
    }, {key: f"Named object {key}" for key in keys}, locked
...
class WorkbookNoteTests(unittest.TestCase):
    ...
    def test_scalar_budget_is_explicit_and_scope_and_paths_are_never_cut(self):
        ...
        locked["containerVersion"]["tag"][0]["p" * 700] = "a" * 600
        operation = source["operations"][0]
        operation["changes"] = [{"object_key": "tag:1", "json_path": "$." + "p" * 700,
                                 "before_source_sha256": stable_hash(locked, 64), "after": "b" * 601}]
        operation["action_payload_sha256"] = stable_hash(operation, 64)
        text = render(_recommendation_rows(source, names, locked, stable_hash(locked, 64))[0])["model"]["comments"][0]["text"]
    ...
    def test_large_payload_omissions_are_explicit_and_exact_record_is_unchanged(self):
        ...
        operation["action_payload_sha256"] = stable_hash(operation, 64)
        original = copy.deepcopy(source)
        row = _recommendation_rows(source, names, locked, stable_hash(locked, 64))[0]
        result = render(row)
        ...
        self.assertNotIn("before", operation["changes"][0])
        self.assertEqual("a" * 82000, row["locked"]["technical_note"]["changes"][0]["before"])
        self.assertEqual(original, source)
    ...
    def test_relevant_nested_changes_action_kinds_and_redaction(self):
        source, names, locked = fixture()
        operation = source["operations"][0]
        locked["containerVersion"]["tag"][0]["parameter"] = [
            {"key": "mode", "value": "old"}, {"key": "token", "value": "private-value"}]
        operation.update(
            changes=[{"object_key": "tag:1", "json_path": "$.parameter",
                      "before_source_sha256": stable_hash(locked, 64),
                      "after": [{"key": "mode", "value": "new"}, {"key": "token", "value": "private-value"}]}],
            additions=[{"object_key": "tag:2", "json_path": "$.priority", "value": 0}],
            removals=[{"object_key": "tag:3", "json_path": "$.notes", "before_source_sha256": stable_hash(locked, 64)}],
    ...
    def test_500_short_values_have_explicit_bounded_detail_and_complete_paths(self):
        ...
        locked["containerVersion"]["tag"][0]["parameter"] = before
        operation["changes"] = [{"object_key": "tag:1", "json_path": "$.parameter",
                                 "before_source_sha256": stable_hash(locked, 64), "after": after}]
        ...
        self.assertEqual(original, source)
        self.assertNotIn("before", operation["changes"][0])
        self.assertEqual(before, row["locked"]["technical_note"]["changes"][0]["before"])
        self.assertEqual(result["model"]["comments"][0], result["model"]["comments"][1])
    ...
    def test_public_and_sensitive_changes_both_keep_their_exact_paths(self):
        ...
        operation = source["operations"][0]
        locked["containerVersion"]["tag"][0]["parameter"] = [
            {"key": "mode", "value": "old"}, {"key": "token", "value": "first-private-value"}]
        operation["changes"] = [{"object_key": "tag:1", "json_path": "$.parameter",
                                 "before_source_sha256": stable_hash(locked, 64),
